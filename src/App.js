...
export default function App() {
  ...
  const changePage = async (pageNumber) => {
    ...
    let url;
    if (!keyword)
      url = `https://api.themoviedb.org/3/movie/now_playing?api_key=${API_KEY}&language=en-US&page=${currentPage}&query=${keyword}`;
    else
      url = `https://api.themoviedb.org/3/search/movie?api_key=${API_KEY}&query=${keyword}&page=${currentPage}`;
    let response = await fetch(url);
    let data = await response.json();
    console.log(keyword);
    setMovieList(data.results);
